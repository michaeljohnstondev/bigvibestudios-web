/**
 * Snappled share pages with real Open Graph tags.
 *
 * Facebook, WhatsApp, iMessage and Discord all fetch a shared URL with a
 * crawler that does not execute JavaScript. A static page that loads its
 * content client-side therefore unfurls identically for every snapple,
 * with no thumbnail — which is what this replaces.
 *
 * The snapple data lives in the `snapplepark` Firebase project, and a
 * Hosting rewrite cannot target a function in another project. An ordinary
 * cross-project HTTPS call can, so this fetches `getShareCard` over the
 * wire and bakes the result into the markup.
 */
...
const CARD_ENDPOINT =
  'https://us-central1-snapplepark.cloudfunctions.net/getShareCard';

const SITE = 'https://bigvibestudios.com';
  ...
  if (!id) {
    ...
    try {
      const response = await fetch(
        CARD_ENDPOINT + '?id=' + encodeURIComponent(id));
      ...
      // Lookup failed, but the page still renders and the client retries
      // the fetch itself. Only the unfurl degrades.
